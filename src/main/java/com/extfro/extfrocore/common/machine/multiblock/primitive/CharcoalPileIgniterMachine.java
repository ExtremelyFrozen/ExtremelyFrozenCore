...
import com.extfro.extfrocore.common.item.behavior.LighterBehavior;
...
public class CharcoalPileIgniterMachine extends WorkableMultiblockMachine implements IWorkable {
    ...
    public void clientTick() {
        ...
        if (isActive()) {
            ...
            float xPos = facing.getStepX() * 0.76F + pos.getX() + 0.25F + EFValues.RNG.nextFloat() / 2.0F;
            float yPos = facing.getStepY() * 0.76F + pos.getY() + 0.25F;
            float zPos = facing.getStepZ() * 0.76F + pos.getZ() + 0.25F + EFValues.RNG.nextFloat() / 2.0F;

            float ySpd = facing.getStepY() * 0.1F + 0.01F * EFValues.RNG.nextFloat();
            float horSpd = 0.03F * EFValues.RNG.nextFloat();
            float horSpd2 = 0.03F * EFValues.RNG.nextFloat();

            if (EFValues.RNG.nextFloat() < 0.1F) {
