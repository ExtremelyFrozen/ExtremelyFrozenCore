...
import com.extfro.extfrocore.common.machine.multiblock.electric.CleanroomMachine;
...
public class CleanroomLogic extends RecipeLogic implements IWorkable {
    ...
    protected boolean consumeEnergy() {
        ...
        var tier = Mth.clamp(getMachine().getTier(), EFValues.ULV, EFValues.MAX);
        // use 3/16th an amp when fully clean otherwise 15/16th an amp during cleaning
        long energyToDrain = cleanroomTrait.isActive() ? Math.max(8, (3 * EFValues.V[tier] / 16)) :
                EFValues.VA[tier];
