...
import com.extfro.extfrocore.api.fluids.GTFluid;
import com.extfro.extfrocore.api.fluids.store.FluidStorage;
import com.extfro.extfrocore.api.fluids.store.FluidStorageKey;
...
public class GTModels {
    ...
    public static final ResourceLocation BLANK_TEXTURE = ExtForCore.id("block/void");
    ...
    public static void cellModel(DataGenContext<Item, ? extends Item> ctx, RegistrateItemModelProvider prov) {
        ...
        prov.generated(ctx::getEntry, prov.modLoc("item/%s/base".formatted(prov.name(ctx))))
                .override().predicate(ExtForCore.id("fluid_cell"), 0)
                .model(new ModelFile.UncheckedModelFile(prov.modLoc("item/%s_empty".formatted(prov.name(ctx)))))
                .end()
                .override().predicate(ExtForCore.id("fluid_cell"), 1)
    ...
    public static final ResourceLocation CUBE_ALL_EMISSIVE = ExtForCore.id("block/cube/emissive/all");
    ...
    public static NonNullBiConsumer<DataGenContext<Block, LampBlock>, RegistrateBlockstateProvider> lampModel(DyeColor color,
        ...
        return (ctx, prov) -> {
            ...
            var bloomModel = prov.models().withExistingParent(ctx.getName() + "_bloom",
                    border ? ExtForCore.id("block/cube_2_layer/all") : CUBE_ALL_EMISSIVE);
            if (border) {
            ...
            prov.simpleBlock(ctx.getEntry(),
                    prov.models()
                            .withExistingParent("%s_machine_casing".formatted(tierName),
                                    ExtForCore.id("block/cube/tinted/bottom_top"))
                            .texture("bottom", ExtForCore.id("block/casings/voltage/%s/bottom".formatted(tierName)))
                            .texture("top", ExtForCore.id("block/casings/voltage/%s/top".formatted(tierName)))
                            .texture("side", ExtForCore.id("block/casings/voltage/%s/side".formatted(tierName))));
        };
    }
    ...
    public static NonNullBiConsumer<DataGenContext<Block, Block>, RegistrateBlockstateProvider> createHermeticCasingModel(String tierName) {
        return (ctx, prov) -> {
            prov.simpleBlock(ctx.getEntry(), prov.models()
                    .withExistingParent("%s_hermetic_casing".formatted(tierName), ExtForCore.id("block/hermetic_casing"))
                    .texture("bot_bottom", ExtForCore.id("block/casings/voltage/%s/bottom".formatted(tierName)))
                    .texture("bot_top", ExtForCore.id("block/casings/voltage/%s/top".formatted(tierName)))
                    .texture("bot_side", ExtForCore.id("block/casings/voltage/%s/side".formatted(tierName))));
        };
    }
    ...
    public static NonNullBiConsumer<DataGenContext<Block, Block>, RegistrateBlockstateProvider> createSteamCasingModel(String material) {
        return (ctx, prov) -> {
            prov.simpleBlock(ctx.getEntry(), prov.models().cubeBottomTop(ctx.getName(),
                    ExtForCore.id("block/casings/steam/%s/side".formatted(material)),
                    ExtForCore.id("block/casings/steam/%s/bottom".formatted(material)),
                    ExtForCore.id("block/casings/steam/%s/top".formatted(material))));
        };
    }
    ...
    public static NonNullBiConsumer<DataGenContext<Block, CoilBlock>, RegistrateBlockstateProvider> createCoilModel(ICoilType coilType) {
        return (ctx, prov) -> {
            ...
            ModelFile active = prov.models().withExistingParent(name + "_active", ExtForCore.id("block/cube_2_layer/all"))
    ...
    public static NonNullBiConsumer<DataGenContext<Block, BatteryBlock>, RegistrateBlockstateProvider> createBatteryBlockModel(IBatteryData batteryData) {
        return (ctx, prov) -> {
            prov.simpleBlock(ctx.getEntry(), prov.models().cubeBottomTop(ctx.getName(),
                    ExtForCore.id("block/casings/battery/" + batteryData.getBatteryName() + "/side"),
                    ExtForCore.id("block/casings/battery/" + batteryData.getBatteryName() + "/top"),
                    ExtForCore.id("block/casings/battery/" + batteryData.getBatteryName() + "/top")));
        };
    }
    ...
    public static NonNullBiConsumer<DataGenContext<Block, Block>, RegistrateBlockstateProvider> createCleanroomFilterModel(IFilterType type) {
        return (ctx, prov) -> {
            prov.simpleBlock(ctx.getEntry(), prov.models()
                    .cubeAll(ctx.getName(), ExtForCore.id("block/casings/cleanroom/" + type.getSerializedName())));
        };
    }
    ...
    public static NonNullBiConsumer<DataGenContext<Block, ActiveBlock>, RegistrateBlockstateProvider> createFireboxModel(BoilerFireboxType type) {
        return (ctx, prov) -> {
            ...
            ModelFile active = prov.models().withExistingParent(name + "_active", ExtForCore.id("block/fire_box_active"))
            ...
            for (FluidStorageKey key : FluidStorageKey.allKeys()) {
                ...
                if (fluid instanceof GTFluid gtFluid) {
                    ...
                    try (BufferedReader reader = Minecraft.getInstance().getResourceManager()
                            .openAsReader(ExtForCore.id("models/item/bucket/bucket.json"))) {
