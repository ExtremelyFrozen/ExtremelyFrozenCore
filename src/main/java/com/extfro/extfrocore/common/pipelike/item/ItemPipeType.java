package com.extfro.extfrocore.common.pipelike.item;
...
import com.extfro.extfrocore.api.pipenet.IMaterialPipeType;
...
public enum ItemPipeType implements IMaterialPipeType<ItemPipeProperties> {
    ...
    public static final ResourceLocation TYPE_ID = ExtForCore.id("item");
    ...
    public PipeModel createPipeModel(PipeBlock<?, ?, ?> block, Material material, GTBlockstateProvider provider) {
        ResourceLocation sideTexture = ExtForCore.id("block/pipe/pipe_side");
        ResourceLocation endTexture = ExtForCore.id("block/pipe/pipe_%s_in"
        ...
        if (isRestrictive()) {
            model.setSideOverlay(ExtForCore.id("block/pipe/pipe_restrictive"));
        }
        return model;
    }
}
