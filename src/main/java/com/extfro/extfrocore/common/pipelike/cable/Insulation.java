package com.extfro.extfrocore.common.pipelike.cable;
...
public enum Insulation implements IMaterialPipeType<WireProperties> {
    ...
    public static final ResourceLocation TYPE_ID = ExtForCore.id("insulation");
    ...
    public PipeModel createPipeModel(PipeBlock<?, ?, ?> block, Material material, GTBlockstateProvider provider) {
        ...
        PipeModel model = new PipeModel(block, provider, thickness,
                isCable ? ExtForCore.id("block/cable/insulation_5") : side, end);
        ...
        if (isCable) {
            model.setEndOverlay(ExtForCore.id("block/cable/insulation_%s".formatted(insulationLevel)));
        }
        return model;
    }
}
