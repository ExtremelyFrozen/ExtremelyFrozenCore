...
import com.extfro.extfrocore.integration.map.IWaypointHandler;
...
public class JourneymapWaypointHandler implements IWaypointHandler {
    ...
    public void setWaypoint(String key, String name, int color, ResourceKey<Level> dim, int x, int y, int z) {
        Waypoint waypoint = WaypointFactory.createClientWaypoint(ExtForCore.MOD_ID, new BlockPos(x, y, z),
                name, dim, true);
        waypoint.setColor(color);
        waypoints.put(key, waypoint);
        try {
            JourneyMapPlugin.getJmApi().addWaypoint(ExtForCore.MOD_ID, waypoint);
        } catch (Exception e) {
            // It never actually throws anything...
            ExtForCore.LOGGER.error("Failed to enable waypoint with name {}", name, e);
        }
    }
    ...
    public void removeWaypoint(String key) {
        ...
        if (removed != null) {
            JourneyMapPlugin.getJmApi().removeWaypoint(ExtForCore.MOD_ID, removed);
        }
    }
}
