package com.extfro.extfrocore.integration.jade;
...
import com.extfro.extfrocore.integration.jade.provider.*;
...
public class GTJadePlugin implements IWailaPlugin {
    ...
    static {
        GTMaterialItems.TOOL_ITEMS.columnMap().forEach((type, map) -> {
            ...
            HarvestToolProvider.registerHandler(SimpleToolHandler.create(ExtForCore.id(type.name), tools, true));
        });
    }
}
