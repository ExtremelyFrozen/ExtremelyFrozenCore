...
public class VirtualEnderRegistry extends SavedData {
    ...
    private static final String DATA_ID = ExtForCore.MOD_ID + ".virtual_entry_data";
    ...
    public static VirtualEnderRegistry getInstance() {
        if (data == null) {
            var server = ExtForCore.getMinecraftServer();
            if (server != null) {
                data = server.overworld().getDataStorage().computeIfAbsent(
                        new Factory<>(VirtualEnderRegistry::new, VirtualEnderRegistry::new), DATA_ID);
            }
        }
    ...
    public static void release() {
        if (data != null) {
            data = null;
            ExtForCore.LOGGER.debug("VirtualEnderRegistry has been unloaded");
        }
    }
    ...
    public void deleteEntry(@Nullable UUID owner, EntryTypes<?> type, String name) {
        ...
        ExtForCore.LOGGER.warn("Attempted to delete {} entry {} of type {}, which does not exist",
