    ...
    private static final LoadingCache<String, byte[]> CACHE = CacheBuilder.newBuilder()
            ...
            .build(CacheLoader.from(urlString -> {
                try {
                    ...
                    boolean singleplayer = ExtForCore.getMinecraftServer().isSingleplayer() &&
                            !ExtForCore.getMinecraftServer().isPublished();
                    ...
                    try (InputStream stream = url.openStream()) {
                        ...
                        ExtForCore.LOGGER.debug("Downloaded image {}! Executing callback", url);
                        return image;
                    } catch (IOException e) {
                        ExtForCore.LOGGER.error("Could not load image {}", url, e);
                        return NULL_MARKER;
                    } finally {
                        downloading = false;
                    }
