...
public class MultiblockWorldSavedData extends SavedData {
    ...
    public static MultiblockWorldSavedData getOrCreate(ServerLevel serverLevel) {
        return serverLevel.getDataStorage().computeIfAbsent(
                new Factory<>(MultiblockWorldSavedData::new, MultiblockWorldSavedData::new),
                "gtceu_multiblock");
    }
    ...
    private final static ThreadFactory THREAD_FACTORY = new ThreadFactoryBuilder()
            .setNameFormat("ExtForCore Multiblock Async Thread-%d")
            .setDaemon(true)
            .build();
    ...
    private void searchingTask() {
        try {
            if (!ExtForCore.canGetServerLevel()) return;
            IN_SERVICE.set(true);
            for (var controller : controllers) {
                try {
                    controller.asyncCheckPattern(periodID);
                } catch (Throwable e) {
                    ExtForCore.LOGGER.error("Error while assembling multiblock {}: {}", controller, e.getMessage());
                }
            }
        } catch (Throwable e) {
            ExtForCore.LOGGER.error("Error while assembling multiblocks: {}", e.getMessage());
        } finally {
    ...
    public static boolean isThreadService() {
        return IN_SERVICE.get() && ExtForCore.canGetServerLevel();
    }
