...
public abstract class PipeNetWalker<T extends IPipeNode<?, ?>, NodeDataType, Net extends PipeNet<NodeDataType>> {
    ...
    public void traversePipeNet(int maxWalks) {
        ...
        root.walked.clear();
        if (i >= maxWalks)
            ExtForCore.LOGGER.warn("The walker reached the maximum amount of walks {}", i);
        invalid = true;
    }
    ...
    private boolean checkPos() {
        ...
        if (currentPipe == null) {
            ...
            if (!(thisPipe instanceof IPipeNode<?, ?>)) {
                ExtForCore.LOGGER.error("PipeWalker expected a pipe, but found {} at {}", thisPipe, currentPos);
                return false;
            }
