...
import com.extfro.extfrocore.api.data.worldgen.bedrockfluid.BedrockFluidVeinSavedData;
import com.extfro.extfrocore.api.data.worldgen.bedrockfluid.FluidVeinWorldEntry;
import com.extfro.extfrocore.api.data.worldgen.bedrockore.BedrockOreVeinSavedData;
...
public abstract class ProspectorMode<T> {
    ...
    public static ProspectorMode<FluidInfo> FLUID = new ProspectorMode<>("metaitem.prospector.mode.fluid", 1) {
        ...
        public void drawSpecialGrid(GuiGraphics graphics, FluidInfo[] items, int x, int y, int width, int height) {
            if (items.length > 0) {
                ...
                float drawnU = (float) FillDirection.DOWN_TO_UP.getDrawnU(progress);
                float drawnV = (float) FillDirection.DOWN_TO_UP.getDrawnV(progress);
                float drawnWidth = (float) FillDirection.DOWN_TO_UP.getDrawnWidth(progress);
    ...
    public static ProspectorMode<OreInfo> BEDROCK_ORE = new ProspectorMode<>("metaitem.prospector.mode.bedrock_ore",
        ...
        public OreInfo deserialize(FriendlyByteBuf buf) {
            ...
            return new OreInfo(
                    EFAPI.materialManager.getMaterial(materialId),
