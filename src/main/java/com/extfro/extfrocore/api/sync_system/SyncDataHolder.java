        ...
        for (var field : fieldsToCheck) {
            ...
            if (readingClientFields) {
                try {
                    ...
                    ExtForCore.LOGGER.error("Sync: Error while invoking change listener for field {}", field.fieldName);
                    ExtForCore.LOGGER.error(e);
                }
    ...
    private static Tag serializeField(HolderLookup.Provider registries, ISyncManaged holder, FieldSyncData field,
        ...
        try {
            ...
            if (field.transformer != null) {
                ...
                ExtForCore.LOGGER.error("Sync: Failed to serialize field {} in class {}: Missing value transformer for {}",
                        field.fieldName, holder.getClass().getName(), field.type);
            }

        } catch (Exception e) {
            ExtForCore.LOGGER.error("Sync: Failed to serialize field {}", field.fieldName);
            ExtForCore.LOGGER.error(e);
        }
        ...
        try {
            if (field.transformer != null) {
                ...
                try {
                    ...
                    ExtForCore.LOGGER.error("Sync: failed to perform VarHandle set: unsupported op {} {}",
                            field.fieldName, field.handle.toString());
                }
            } else if (field.isSyncManaged && savedValue instanceof CompoundTag compound) {
                if (currentVal == null) {
                    ExtForCore.LOGGER.error("Sync: ISyncManaged field was null, cannot instantiate {}",
                            field.fieldName);
                    return;
                }
                ...
                ExtForCore.LOGGER.error("Sync: Failed to deserialize field {} in class {}: Missing value transformer for {}",
                        field.fieldName, holder.getClass().getName(), field.type);
            }
        } catch (Exception e) {
            ExtForCore.LOGGER.error("Sync: Failed to deserialize field {}", field.fieldName);
            ExtForCore.LOGGER.error(e);
        }
    }
