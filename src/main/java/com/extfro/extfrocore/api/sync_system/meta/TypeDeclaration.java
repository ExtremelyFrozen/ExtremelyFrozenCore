...
public class TypeDeclaration {
    ...
    public TypeDeclaration(Type type) {
        ...
        if (type instanceof ParameterizedType parameterizedType) {
            ...
            this.genericTypeArgs = java.util.Arrays.stream(parameterizedType.getActualTypeArguments())
    ...
    public TypeDeclaration getArrayComponentType() {
        if (arrayComponentType == null) {
            throw new IllegalStateException("Not an array type: %s".formatted(rawType));
        }
