package com.extfro.extfrocore.api.data.chemical.material.info;
...
import com.extfro.extfrocore.integration.kjs.events.MaterialIconInfoEventJS;
...
public record MaterialIconType(String name) {
    ...
    public ResourceLocation getBlockTexturePath(@NotNull MaterialIconSet materialIconSet, @Nullable String suffix,
        ...
        if (!ExtForCore.isClientSide() || Minecraft.getInstance() == null ||
        ...
        if (!iconSet.isRootIconset) {
            while (!iconSet.isRootIconset) {
                ResourceLocation location = ExtForCore
        ...
        location = ExtForCore.id(String.format("block/material_sets/%s/%s%s", iconSet.name, this.name, suffix));
        if (suffix.isEmpty()) {
    ...
    public ResourceLocation getBlockModelPath(@NotNull MaterialIconSet materialIconSet, boolean doReadCache) {
        ...
        MaterialIconSet iconSet = materialIconSet;
        // noinspection ConstantConditions
        if (!iconSet.isRootIconset && ExtForCore.isClientSide() && Minecraft.getInstance() != null &&
        ...
        ResourceLocation location = ExtForCore.id(String.format("block/material_sets/%s/%s", iconSet.name, this.name));
    ...
    public ResourceLocation getItemModelPath(@NotNull MaterialIconSet materialIconSet, boolean doReadCache) {
        ...
        ResourceLocation location = ExtForCore.id(String.format("item/material_sets/%s/%s", iconSet.name, this.name));
        ITEM_MODEL_CACHE.put(this, materialIconSet, location);
    ...
    public ResourceLocation getItemTexturePath(@NotNull MaterialIconSet materialIconSet, String suffix,
        ...
        location = ExtForCore.id(String.format("item/material_sets/%s/%s%s", iconSet.name, this.name, suffix));
        if (suffix.isEmpty()) {
