        ...
        if (handlerIO == IO.IN) {
            if (isTransmitter()) {
                ...
                if (machine instanceof IOpticalComputationProvider provider) {
                    ...
                    ExtForCore.LOGGER.error("NotifiableComputationContainer could request CWU/t from its machine!");
                    return 0;
                }
            } else {
        ...
        if (handlerIO == IO.IN) {
            if (isTransmitter()) {
                ...
                if (machine instanceof IOpticalComputationProvider provider) {
                    ...
                    ExtForCore.LOGGER.error("NotifiableComputationContainer could not get maximum CWU/t from its machine!");
                    return 0;
                }
            } else {
                // Ask the attached Transmitter hatch, if it exists
        ...
        if (handlerIO == IO.IN) {
            if (isTransmitter()) {
                ...
                if (machine instanceof IOpticalComputationProvider provider) {
                    ...
                    ExtForCore.LOGGER.error("NotifiableComputationContainer could not test bridge status of its machine!");
                    return false;
                }
            } else {
                // Ask the attached Transmitter hatch, if it exists
