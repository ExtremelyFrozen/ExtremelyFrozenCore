...
import com.extfro.extfrocore.api.data.chemical.material.properties.DustProperty;
...
import com.extfro.extfrocore.api.item.component.ElectricStats;
...
import com.extfro.extfrocore.api.item.datacomponents.GTTool;
import com.extfro.extfrocore.api.item.tool.GTToolType;
import com.extfro.extfrocore.api.item.tool.IGTToolDefinition;
import com.extfro.extfrocore.api.item.tool.TreeFellingHelper;
import com.extfro.extfrocore.api.item.tool.behavior.IToolBehavior;
...
public interface IGTTool extends HeldItemUIMenuType.HeldItemUI, ItemLike {
    ...
    default ModularUI createUI(HeldItemUIMenuType.HeldItemUIHolder holder) {
        for (var behavior : getToolStats().getBehaviors()) {
            if (!(behavior instanceof IToolUIBehavior<?> uiBehavior) || !uiBehavior.openUI(holder.player, holder.hand)) {
                continue;
            }
            return uiBehavior.createUI(holder);
        }
        return null;
    }
