...
public class TooltipsHandler {
    ...
    private static final String ITEM_PREFIX = "item." + ExtForCore.MOD_ID;
    private static final String BLOCK_PREFIX = "block." + ExtForCore.MOD_ID;
    ...
    public static void appendFluidTooltips(FluidStack fluidStack, Consumer<Component> tooltips, TooltipFlag flag,
        ...
        if (!material.isNull()) {
            ...
            if (material.hasProperty(PropertyKey.INGOT)) {
                if (GTUtil.isShiftDown() && amount >= EFValues.L) {
                    long ingots = amount / EFValues.L;
                    long remainder = amount % EFValues.L;
                    String fluidAmount = String.format(" %,d mB = %,d * %d mB", amount, ingots, EFValues.L);
                    if (remainder != 0) {
