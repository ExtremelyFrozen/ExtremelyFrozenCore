...
public class TooltipHelper {
    ...
    public static TextColor rainbowColor(float speed) {
        return TextColor.fromRgb(GradientUtil.toRGB((EFValues.CLIENT_TIME & ((1 << 20) - 1)) * speed, 95f, 60f));
    }
    ...
    public static GTFormattingCode createNewCode(int rate, ChatFormatting... codes) {
        if (rate <= 0) {
            ExtForCore.LOGGER.error("Could not create GT Formatting Code with rate {}, must be greater than zero!", rate);
            return null;
        }
        if (codes == null || codes.length <= 1) {
            ExtForCore.LOGGER.error("Could not create GT Formatting Code with codes {}, must have length greater than one!",
    ...
    public static class GTFormattingCode {
        ...
        public void updateIndex() {
            if (EFValues.CLIENT_TIME % rate == 0) {
