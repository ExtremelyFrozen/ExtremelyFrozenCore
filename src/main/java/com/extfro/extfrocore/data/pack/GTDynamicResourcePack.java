...
import static com.extfro.extfrocore.data.pack.GTDynamicDataPack.writeJson;
...
public class GTDynamicResourcePack implements PackResources {
    ...
    static {
        CLIENT_DOMAINS.addAll(Sets.newHashSet(ExtForCore.MOD_ID, "minecraft", "neoforge", "c", "kubejs"));
    }
    ...
    public static void addResource(ResourceLocation location, byte[] data) {
        if (ConfigHolder.INSTANCE.dev.dumpAssets) {
            Path parent = ExtForCore.EF_FOLDER.resolve("dumped/assets");
    ...
    public @Nullable <T> T getMetadataSection(MetadataSectionSerializer<T> metaReader) {
        if (metaReader == PackMetadataSection.TYPE) {
            return (T) new PackMetadataSection(Component.literal("ExtForCore dynamic assets"),
