...
public final class ChestGenHooks {
        ...
        if (lootEntryItems.containsKey(name)) {
            ...
            for (GTLootEntryItem entry : entryItems) {
                if (ConfigHolder.INSTANCE.dev.debug) {
                    ExtForCore.LOGGER.info("adding {} to lootTable {}", entry, name);
                }
                ...
                try {
                    ...
                    ExtForCore.LOGGER.error("Couldn't add {} to lootTable {}: {}", entry, name, e.getMessage());
                }
            }
        }
    ...
    public static class RandomWeightLootFunction extends LootItemConditionalFunction implements LootItemFunction {
        ...
        public static final LootItemFunctionType<RandomWeightLootFunction> TYPE = GTRegistries.register(
                BuiltInRegistries.LOOT_FUNCTION_TYPE, ExtForCore.id("random_weight"), new LootItemFunctionType<>(CODEC));
