...
public final class ToolRecipeHandler {
    ...
    public static final Int2ReferenceMap<ItemEntry<? extends Item>> powerUnitItems = new Int2ReferenceArrayMap<>(
            EFValues.tiersBetween(EFValues.LV, EFValues.IV),
    ...
    private static void processTool(@NotNull RecipeOutput provider, @NotNull Material material) {
        ...
        if (material.hasFlag(GENERATE_ROD)) {
            ...
            if (material.hasFlag(GENERATE_PLATE)) {
                ...
                ExtForCore.LOGGER.info("Did not find plate for {}, skipping wirecutter, butchery knife recipes",
                        material.getName());
            }
            ...
            ExtForCore.LOGGER.warn("Did not find rod for " + material.getName() +
    ...
    private static void processElectricTool(@NotNull RecipeOutput provider, @NotNull ToolProperty property,
        ...
        final int voltageMultiplier = material.getBlastTemperature() > 2800 ? EFValues.VA[EFValues.LV] :
                EFValues.VA[EFValues.ULV];
        ...
        if (material.hasFlag(GENERATE_PLATE)) {
            ...
            if (property.hasType(GTToolType.BUZZSAW_LV)) {
                ...
                if (material.hasFlag(GENERATE_GEAR)) {
                    ...
                    ExtForCore.LOGGER.warn("Did not find gear for " + material.getName() +
                            ", skipping gear -> buzzsaw blade recipe");
                }
            }
        } else {
            ExtForCore.LOGGER.warn("Did not find plate for " + material.getName() +
        ...
        if (property.hasType(GTToolType.SCREWDRIVER_LV)) {
            if (material.hasFlag(GENERATE_LONG_ROD)) {
                ...
                ExtForCore.LOGGER.warn("Did not find long rod for " + material.getName() +
