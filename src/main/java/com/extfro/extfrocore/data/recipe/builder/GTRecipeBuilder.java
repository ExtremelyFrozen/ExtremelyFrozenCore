package com.extfro.extfrocore.data.recipe.builder;
...
import com.extfro.extfrocore.api.recipe.*;
...
import com.extfro.extfrocore.api.recipe.ingredient.*;
...
import com.extfro.extfrocore.common.recipe.condition.*;
...
import com.extfro.extfrocore.utils.codec.CodecUtils;
...
public class GTRecipeBuilder {
    ...
    public static GTRecipeBuilder ofRaw() {
        return new GTRecipeBuilder(ExtForCore.id("raw"), GTRecipeTypes.DUMMY_RECIPES);
    }

    public GTRecipeBuilder copy(String id) {
        return copy(ExtForCore.id(id));
    }
    ...
    public GTRecipeBuilder duration(int duration) {
        if (duration < 0) {
            ExtForCore.LOGGER.error("Recipe duration must be non negative, id: {}", this.id);
        }
    ...
    public GTRecipeBuilder EUt(long voltage, long amperage) {
        if (voltage == 0) {
            ExtForCore.LOGGER.error("EUt can't be explicitly set to 0, id: {}", id);
        }
        if (amperage < 1) {
            ExtForCore.LOGGER.error("Amperage must be a positive integer, id: {}", id);
        }
    ...
    public GTRecipeBuilder CWUt(int cwu) {
        if (cwu == 0) {
            ExtForCore.LOGGER.error("CWUt can't be explicitly set to 0, id: {}", id);
        }
        var lastPerTick = perTick;
        perTick = true;
        if (cwu > 0) {
    ...
    public GTRecipeBuilder inputItemsRanged(TagPrefix orePrefix, Material material, IntProvider intProvider) {
        ...
        if (item.isEmpty()) {
            ExtForCore.LOGGER.error("Tried to set input ranged item stack that doesn't exist, TagPrefix: {}, Material: {}",
                    orePrefix, material);
        }
    ...
    public GTRecipeBuilder outputItemsRanged(TagPrefix orePrefix, Material material, IntProvider intProvider) {
        var item = ChemicalHelper.get(orePrefix, material, 1);
        if (item.isEmpty()) {
            ExtForCore.LOGGER.error("Tried to set output ranged item stack that doesn't exist, TagPrefix: {}, Material: {}",
                    orePrefix, material);
        }
    ...
    public GTRecipeBuilder circuitMeta(int configuration) {
        if (configuration < 0 || configuration > IntCircuitBehaviour.CIRCUIT_MAX) {
            ExtForCore.LOGGER.error("Circuit configuration must be in the bounds 0 - 32");
        }
    ...
    public GTRecipeBuilder chancedOutput(ItemStack stack, String fraction, int tierChanceBoost) {
        ...
        if (split.length != 2) {
            ExtForCore.LOGGER.error("Fraction was not parsed correctly! Expected format is \"1/3\". Actual: \"{}\".",
        ...
        if (0 >= chance || chance > ChanceLogic.getMaxChancedValue()) {
            ExtForCore.LOGGER.error("Chance cannot be less or equal to 0 or more than {}. Actual: {}.",
                    ChanceLogic.getMaxChancedValue(), chance, new Throwable());
            return this;
        }
        if (chance >= maxChance || maxChance > ChanceLogic.getMaxChancedValue()) {
            ExtForCore.LOGGER.error("Max Chance cannot be less or equal to Chance or more than {}. Actual: {}.",
    ...
    public GTRecipeBuilder inputFluids(FluidStack input) {
        ...
        if (!matStack.isNull() && chance != 0 && chance == maxChance) {
            tempFluidStacks.add(new MaterialStack(matStack, input.getAmount() * EFValues.M / EFValues.L));
        }
            ...
            if (missingIngredientError(i, true, FluidRecipeCapability.CAP, fluid::isEmpty)) {
                ...
                if (!matStack.isNull()) {
                    if (chance == maxChance && chance != 0) {
                        tempFluidStacks.add(new MaterialStack(matStack, fluid.getAmount() * EFValues.M / EFValues.L));
                    }
                }
    ...
    public final GTRecipeBuilder adjacentFluids(boolean isReverse, Fluid... fluids) {
        if (fluids.length > GTUtil.NON_CORNER_NEIGHBOURS.size()) {
            ExtForCore.LOGGER.error("Adjacent fluid condition has too many fluids, not adding to recipe. id: {}", this.id);
            return this;
        }
    ...
    public GTRecipeBuilder adjacentBlocks(boolean isReverse, Block... blocks) {
        if (blocks.length > GTUtil.NON_CORNER_NEIGHBOURS.size()) {
            ExtForCore.LOGGER.error("Adjacent block condition has too many blocks, not adding to recipe. id: {}", this.id);
            return this;
        }
    ...
    public GTRecipeBuilder heraclesQuest(String questId, boolean isReverse) {
        if (!ExtForCore.Mods.isHeraclesLoaded()) {
            ExtForCore.LOGGER.error("Heracles not loaded!");
            return this;
        }
        if (questId.isEmpty()) {
            ExtForCore.LOGGER.error("Quest ID cannot be empty for recipe {}", this.id);
            return this;
        }
    ...
    // if (!ExtForCore.Mods.isGameStagesLoaded()) {
    // ExtForCore.LOGGER.warn("GameStages is not loaded, ignoring recipe condition");
    // return this;
    // }
    ...
    public GTRecipeBuilder ftbQuest(String questId, boolean isReverse) {
        if (!ExtForCore.Mods.isFTBQuestsLoaded()) {
            ExtForCore.LOGGER.error("FTBQuests is not loaded!");
            return this;
        }
        if (questId.isEmpty()) {
            ExtForCore.LOGGER.error("Quest ID cannot be empty for recipe {}", this.id);
        ...
        if (qID == 0L) {
            ExtForCore.LOGGER.error("Quest {} not found for recipe {}", questId, this.id);
            return this;
        }
    ...
    private boolean applyResearchProperty(ResearchData.ResearchEntry researchEntry) {
        ...
        if (researchEntry == null) {
            ExtForCore.LOGGER.error("Research Entry cannot be empty.", new IllegalArgumentException());
            return false;
        }

        if (!generatingRecipes) {
            ExtForCore.LOGGER.error("Cannot generate recipes when using researchWithoutRecipe()",
    ...
    public void save(RecipeOutput output) {
        ...
        if (recipeType != null) {
            if (recipeCategory == null) {
                ExtForCore.LOGGER.error("Recipes must have a category", new IllegalArgumentException());
            } else if (recipeCategory != GTRecipeCategory.DEFAULT && recipeCategory.getRecipeType() != recipeType) {
                ExtForCore.LOGGER.error("Cannot apply Category with incompatible RecipeType",
                        new IllegalArgumentException());
            }
        }
    ...
    protected void warnTooManyIngredients(RecipeCapability<?> capability,
        ...
        if (table.getOrDefault(capability, List.of()).size() + addedEntries > max) {
            ...
            ExtForCore.LOGGER.warn("Recipe {} is trying to add more {} than its recipe type can support, Max {} {}: {}",
    ...
    protected boolean missingIngredientError(int index, boolean isInput,
        ...
        if (empty.getAsBoolean()) {
            ...
            ExtForCore.LOGGER.error("{} {} {} of recipe {} is empty", io, cap.name, size + index, id);
            return true;
        }
        return false;
    }
